...
pub fn hash_slot_map<K: slotmap::Key + std::hash::Hash, V: std::hash::Hash + slotmap::Slottable, H: std::hash::Hasher>(map: &slotmap::SlotMap<K, V>, h: &mut H) {
...
pub fn hash_secondary_map<K: slotmap::Key + std::hash::Hash, V: std::hash::Hash, H: std::hash::Hasher>(map: &slotmap::SecondaryMap<K, V>, h: &mut H) {
    map.iter().for_each(|(k, v)| {
        k.hash(h);
        v.hash(h);
    });
