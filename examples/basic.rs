...
pub trait VertexSize: std::clone::Clone + std::default::Default { fn size_of()->usize; }
pub trait VertexColor { fn set_color(&mut self, color: [u8; 4]); fn set_alpha(&mut self, alpha: u8); fn alpha(&self)->u8; }
pub trait VertexPos2 { fn set_pos(&mut self, pos: [f32; 2]); }
pub trait VertexUV { fn set_uv(&mut self, uv: [f32; 2]); }
impl VertexSize for Vertex { fn size_of()->usize{ std::mem::size_of::<Self>() } }
impl VertexPos2 for Vertex { fn set_pos(&mut self, pos: [f32; 2]) { self.pos = pos; } }
impl VertexUV for Vertex{ fn set_uv(&mut self, uv: [f32; 2]) { self.uv = uv; } }
impl VertexColor for Vertex {
    fn set_color(&mut self, color: [u8; 4]) { self.color = color; }
    fn set_alpha(&mut self, alpha: u8) { self.color[3] = alpha; }
    fn alpha(&self)->u8{ self.color[3] }
}
...
impl<Vertex: VertexSize> Geometry<Vertex> {
    ...
    #[inline(always)]
    pub fn allocate(&mut self, num_vertices: usize, num_indices: usize, def: Vertex)->(&mut [Vertex], &mut [IndexType], IndexType) {
        let old_vertices = self.vertices.len();
        self.vertices.resize(old_vertices + num_vertices, def);
        let old_indices = self.indices.len();
        self.indices.resize(old_indices + num_indices, 0 as IndexType);
        (&mut self.vertices[old_vertices..], &mut self.indices[old_indices..], old_vertices as IndexType)
    }
...
impl<Vertex: VertexSize + VertexPos2 + VertexColor> Geometry<Vertex> {
    ...
    pub fn add_circle_aa(&mut self, pos: Vec2, radius: f32, num_segments: usize, def: Vertex) {
        ...
        for (i, pair) in vs.chunks_mut(2).enumerate() {
            ...
            for (v, p) in pair.iter_mut().zip(&[0.0, 1.0]) {
                v.set_pos([pos.x + cos * (radius - 0.5 + p),
        ...
        for (section_i, section) in is.chunks_mut(9).enumerate() {
            ...
            let indices = [central_vertex,    section_i * 2 + 0, section_n * 2 + 0,
                           section_i * 2 + 0, section_i * 2 + 1, section_n * 2 + 0,
                           section_i * 2 + 1, section_n * 2 + 1, section_n * 2 + 0
            ];
    ...
    pub fn add_circle_outline_aa(&mut self, pos: Vec2, radius: f32, thickness: f32, num_segments: usize, def: Vertex) {
        if thickness > 1.0 {
            ...
            for (i, pair) in vs.chunks_mut(4).enumerate() {
                ...
                let cos = angle.cos();
                let sin = angle.sin();
                for (v, p) in pair.iter_mut().zip(&[(-ht - 1.0, 0), (-ht, 255), (ht, 255), (ht + 1.0, 0)]) {
                    v.set_pos([
            ...
            for (section_i, section) in is.chunks_mut(18).enumerate() {
                let section_n = (section_i + 1) % num_segments;
                let indices = [
                    section_i * 4 + 0, section_i * 4 + 1, section_n * 4 + 0,
                    section_i * 4 + 1, section_n * 4 + 1, section_n * 4 + 0,

                    section_i * 4 + 1, section_i * 4 + 2, section_n * 4 + 1,
                    section_i * 4 + 2, section_n * 4 + 2, section_n * 4 + 1,

                    section_i * 4 + 2, section_i * 4 + 3, section_n * 4 + 2,
                    section_i * 4 + 3, section_n * 4 + 3, section_n * 4 + 2
                ];
                for (dest, src) in section.iter_mut().zip(&indices) {
                    *dest = first + *src as IndexType;
                }
            ...
            for (i, pair) in vs.chunks_mut(4).enumerate() {
                ...
                let cos = angle.cos();
                let sin = angle.sin();
                for (v, p) in pair.iter_mut().zip(&[(-1.0, 0), (0.0, (255.0 * thickness) as u8), (1.0, 0)]) {
                    v.set_pos([
                        pos[0] + cos * (radius + p.0),
                        pos[1] + sin * (radius + p.0),
                    ]);
                    v.set_alpha(p.1);
                }
    ...
    pub fn add_polyline_aa(&mut self, points: &[Vec2], color: [u8; 4], closed: bool, thickness: f32) {
        ...
        temp_points.resize(points.len() * if thick_line { 4 } else { 2 }, Vec2::new(0.0, 0.0));
        for i1 in 0..count {
        ...
        if !thick_line {
            if !closed {
                ...
                temp_points[(points.len()-1)*2+0].x = points[points.len()-1].x + temp_normals[points.len()-1].x * gradient_size;
                temp_points[(points.len()-1)*2+0].y = points[points.len()-1].y + temp_normals[points.len()-1].y * gradient_size;
                temp_points[(points.len()-1)*2+1].x = points[points.len()-1].x - temp_normals[points.len()-1].x * gradient_size;
                temp_points[(points.len()-1)*2+1].y = points[points.len()-1].y - temp_normals[points.len()-1].y * gradient_size;
            }
            ...
            if !closed {
                temp_points[0].x = points[0].x + temp_normals[0].x * (half_inner_thickness + gradient_size);
                temp_points[0].y = points[0].y + temp_normals[0].y * (half_inner_thickness + gradient_size);
                ...
                temp_points[3].x = points[0].x - temp_normals[0].x * (half_inner_thickness + gradient_size);
                temp_points[3].y = points[0].y - temp_normals[0].y * (half_inner_thickness + gradient_size);

                temp_points[(points.len()-1)*4+0].x = points[points.len()-1].x + temp_normals[points.len()-1].x * (half_inner_thickness + gradient_size);
                temp_points[(points.len()-1)*4+0].y = points[points.len()-1].y + temp_normals[points.len()-1].y * (half_inner_thickness + gradient_size);
                temp_points[(points.len()-1)*4+1].x = points[points.len()-1].x + temp_normals[points.len()-1].x * (half_inner_thickness);
                temp_points[(points.len()-1)*4+1].y = points[points.len()-1].y + temp_normals[points.len()-1].y * (half_inner_thickness);
                temp_points[(points.len()-1)*4+2].x = points[points.len()-1].x - temp_normals[points.len()-1].x * (half_inner_thickness);
                temp_points[(points.len()-1)*4+2].y = points[points.len()-1].y - temp_normals[points.len()-1].y * (half_inner_thickness);
                temp_points[(points.len()-1)*4+3].x = points[points.len()-1].x - temp_normals[points.len()-1].x * (half_inner_thickness + gradient_size);
                temp_points[(points.len()-1)*4+3].y = points[points.len()-1].y - temp_normals[points.len()-1].y * (half_inner_thickness + gradient_size);
            }
...
impl<Vertex: VertexSize + VertexPos2 + VertexUV> Geometry<Vertex> {
    pub fn add_rect_uv(&mut self, rect: [f32; 4], uv: [f32; 4], def: Vertex)->&mut [Vertex] {
        ...
        for ((dest, pos), uv) in vs.iter_mut().zip([
            [rect[0], rect[1]],
            [rect[2], rect[1]],
            [rect[2], rect[3]],
            [rect[0], rect[3]],
    ...
    pub fn new(ctx: &mut Context) -> Stage {
        ...
        let pipeline = Pipeline::with_params(
            ...
            miniquad::PipelineParams {
                color_blend: Some((miniquad::Equation::Add,
        ...
        while num_created < 80 {
            ...
            let radius = 8.0 + 16.0 * unsafe{ miniquad::rand() as f32 / miniquad::RAND_MAX as f32 };
            let d = (sd_terrain(vec2(x, y)) - radius).min(world.sample_distance(vec2(x, y), terrain, 0xffffffff, 32.0));
            if d < 0.0 {
...
impl EventHandler for Stage {
    fn update(&mut self, _ctx: &mut Context) {
        ...
        while remaining_dt > time_step {
            ...
            for b in active_bodies.into_iter() {
                self.world.body_set_force_by_acceleration(b, vec2(0., 800.0));
            }
            ...
            self.world.simulate_physics(distance_func, normal_func, dt, &mut started_contacts, &mut finished_contacts);
    ...
    fn mouse_button_down_event(&mut self, _c: &mut Context, button: MouseButton, x: f32, y: f32) {
        ...
        match button {
            MouseButton::Left => {
                self.world.find_exact(&mut bodies, vec2(x, y), circle2d::Shape::Circle(1.0), 1, &sd_terrain);
                self.grabbed_body = bodies.first().copied();
            }
            MouseButton::Right => {
                let r = 100.0;
                self.world.find_exact(&mut bodies, vec2(x, y), circle2d::Shape::Circle(r), 1, &sd_terrain);
                for b in bodies.into_iter() {
                    let delta = self.world.body_position(b) - vec2(x, y);
                    self.world.body_set_velocity(b, delta.normalized() * (1.0 - delta.magnitude() / r) * 3000.0);
                }
            }
            _ => {}
        }
    ...
    fn draw(&mut self, c: &mut Context) {
        ...
        let def = Vertex{ pos: [0., 0.], uv: [0., 0.], color: [255,255,255,255]};
        self.geometry.add_rect_uv([0., 0., self.background_image.width as _, self.background_image.height as _], [0.0, 0.0, 1.0, 1.0], def);
        ...
        for k in self.world.bodies() {
            ...
            let contact_count: u32 = self.world.persistent_contacts.keys().map(|t| {
                let mut result = 0;
                if t.0 == k {
                    result += 1;
                }
            ...
            let color = match (self.world.body_type(k) == circle2d::BodyType::Static || self.world.body_is_sleeping(k), contact_count) {
            ...
            match shape {
                circle2d::Shape::Circle(radius) => {
                    ...
                    if (shape_flags & self.world.collision_shape_flags) == 0 {
                        self.geometry.add_circle_outline_aa(pos, radius - 1.0, 2.0, 32, Vertex{ color, ..def });
                    } else {
                        self.geometry.add_circle_aa(pos, radius, 32, Vertex{ color, ..def });
                    }
                    let dir = Vec2::new(rotation.cos(), rotation.sin());
                    self.geometry.add_polyline_aa(&[pos, pos + dir * radius], [color[0]/2, color[1]/2, color[2]/2, 255], false, 2.0);
                },
                _ => {}
            }
            // velocity vector
            self.geometry.add_polyline_aa(&[pos, pos + vel / 4.0], [color[0]/2, color[1]/2, color[2]/2, 255], false, 4.0);
            self.geometry.add_polyline_aa(&[pos, pos + vel / 4.0], color, false, 2.0);
        }

        for (_k, p) in &self.world.persistent_contacts {
            let color = [50, 50, 0, 255];
            for point in p.points.iter() {
                self.geometry.add_polyline_aa(&[point.position, point.position + point.normal * 5.0], color, false, 1.0);
                self.geometry.add_circle_aa(point.position, 1.5, 8, Vertex{ color: [25,25,0,255], ..def });
            }
        }
        ...
        if self.index_buffer.is_none() {
            self.index_buffer = Some(miniquad::Buffer::stream(c, miniquad::BufferType::IndexBuffer, self.geometry.indices.capacity() * std::mem::size_of::<IndexType>()));
        }
        ...
        if self.vertex_buffer.is_none() {
            self.vertex_buffer = Some(miniquad::Buffer::stream(c, miniquad::BufferType::VertexBuffer, self.geometry.vertices.capacity() * std::mem::size_of::<Vertex>()));
        }
        self.index_buffer.unwrap().update(c, self.geometry.indices.as_slice());
        self.vertex_buffer.unwrap().update(c, self.geometry.vertices.as_slice());
